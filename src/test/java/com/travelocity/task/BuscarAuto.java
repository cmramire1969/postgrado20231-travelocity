...
public class BuscarAuto {
    ...
    public static Performable conOrigenDestino(String origen, String destino) {
        return Task.where("{0} selecciona un auto en"+ origen + "hasta '" + destino + "'",
                ...
                Enter.theValue(origen).into(BUT_AUTO_ORIGEN2).thenHit(Keys.DOWN).thenHit(Keys.ENTER),
                //Enter.theValue(destino).into(BUT_AUTO_ORIGEN3).thenHit(Keys.ENTER),
                Click.on(BUT_AUTO_DESTINO),
                Enter.theValue(destino).into(BUT_AUTO_DESTINO2).thenHit(Keys.DOWN).thenHit(Keys.ENTER),
                Click.on(BUT_FECHA),
